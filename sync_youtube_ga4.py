...
структурі (namespaced, щоб нічого не зламати):
  ...
  YouTube (YouTube Analytics API):
    ...
    yt_impressions, yt_ctr            — impressions і CTR (%)
...
def fetch_youtube_metrics(start: dt.date, end: dt.date, month_key: str) -> dict[str, Any]:
    ...
    # 2) Impressions / CTR — окрема "таблиця" метрик, YouTube API не дозволяє
    #    комбінувати їх з базовими метриками в одному запиті.
    try:
        impr = yta.reports().query(
            ids=channel_ref,
            startDate=start.isoformat(),
            endDate=end.isoformat(),
    ...
    impressions, ctr = (impr["rows"][0] if impr.get("rows") else (0, 0))
    ...
    return {
        ...
        "yt_impressions": int(impressions),
        "yt_ctr": round(float(ctr), 2),
    }
...
def main() -> int:
    ...
    log.info(
        "YouTube OK: views(video/shorts)=%s/%s subs_new=%s subs_total(now)=%s watch_h=%s impr=%s ctr=%s%%",
        yt["yt_views_video"], yt["yt_views_shorts"], yt["yt_subs_new"], yt["yt_subs_total"],
        yt["yt_watch_hours"], yt["yt_impressions"], yt["yt_ctr"],
    )
